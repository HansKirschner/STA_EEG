function clusterInfo=getEEG_clusterSize(EEG_dat, connectMat, thresh, tail, mask);
...
% mask: 1/0 electrode ID x timepoint matrix indicating a subregion within
...
% as I can tell)accurate. here is the post-hoc description of whats happening:
...
if nargin<5
    mask=true(size(squeeze(tmpSigMap)));
end

tmpSigMap=squeeze(tmpSigMap).*mask;
